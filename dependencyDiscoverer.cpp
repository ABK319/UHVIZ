...
2270620
SP Exercise 2
This is my own work except the code which was provided as a part of the exercise.
*/
...
private:
  ...
  std::mutex mutex;
public:
  bool ended(std::string s) {
    std::unique_lock<std::mutex> lock(mutex);
    return t.find(s) == t.end();
  }
  void insert(std::pair<std::string, std::list<std::string>> s) {
    std::unique_lock<std::mutex> lock(mutex);
    t.insert(s);
  }

  std::list<std::string> &operator[](std::string s) {
    std::unique_lock<std::mutex> lock(mutex);
    return t[s];
  }
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (!theTable.ended(name)) { continue; }
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  void exThread() {
    while (!workQ.empty()) {
      std::string filename = workQ.process_front();
      if(filename.empty()) {
        break;
      }
      if (theTable.ended(filename)) {
  ...
  int main(int argc, char *argv[]) {
    ...
    // read CRAWLER_THREADS
    char* clawler_threads = getenv("CRAWLER_THREADS");
    int thread_num = 2;
    if (clawler_threads != NULL) {
      thread_num = atoi(clawler_threads);
    }
